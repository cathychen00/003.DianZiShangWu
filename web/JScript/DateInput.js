...
function NumOnly(e)
{
	...
	return ((KeyCode == 8) || (KeyCode == 9) || (KeyCode == 37) || (KeyCode == 39) || (KeyCode == 46) || ((KeyCode > 47) && (KeyCode < 58)));
}
...
function storedMonthObject(DateFormat, DateYear, DateMonth, DateDay)
{
	...
	{
		if (/MM?\/DD?\/Y{2,4}/.test(DateFormat))
			var FirstPart = this.monthPad + '/' + this.dayPad + '/';
		else if (/DD?\/MM?\/Y{2,4}/.test(DateFormat))
			var FirstPart = this.dayPad + '/' + this.monthPad + '/';
		else if (/DD?-((MON)|(MMM))-Y{2,4}/.test(DateFormat))
			var FirstPart = this.dayPad + '-' + this.monthShort + '-';
		else if (/((MON)|(MMM))-DD?-Y{2,4}/.test(DateFormat))
			var FirstPart = this.monthShort + '-' + this.dayPad + '-';
...
function neighborYearObject(ParentObject, IDText, DateYear, DateMonth)
{
	...
	this.go = new Function(ParentObject.objName + '.getCalendar().style.zIndex=++ZCounter;' + ParentObject.objName + '.setDisplayed(this.yearValue,this.monthIndex);');
	if (ParentObject.formNumber >= 0)
		this.getButton().title = this.yearValue;
}
...
function calendarObject(DateName, DateFormat, DefaultDate)
{
	...
	{
		// Returns the index (0-11) of the supplied month abbreviation
	...
	{
		// Notifies the user about their bad default date, and sets the current system date
...
function DateInput(DateName, DateFormat, DefaultDate, Dir)
{
	var ImageURL = Dir + 'calendar.gif';
	var NextURL = Dir + 'calendar_next.gif';
	var PrevURL = Dir + 'calendar_prev.gif';
	var NextYearURL = Dir + 'calendar_nextYear.gif';
	var PrevYearURL = Dir + 'calendar_prevYear.gif';
	...
	else
	{
		...
		else if ((/^YYYYMMDD$/i.test(DateFormat)) || (/^YYYY-MM-DD$/i.test(DateFormat)) || (/^((MM?)|(DD?))\/((MM?)|(DD?))\/Y{2,4}$/i.test(DateFormat)) || (/^((DD?)|((MON)|(MMM)))-((DD?)|((MON)|(MMM)))-Y{2,4}$/i.test(DateFormat)))
		...
		with (document)
		{
			...
			for (var w=0;w<7;w++)
				writeln('<td width="18" align="center" class="calendarDateInput" style="height:' + CellHeight + ';width:' + CellWidth + ';font-weight:bold;border-top:1px solid dimgray;border-bottom:1px solid dimgray;">' + WeekDays[w] + '</td>');
