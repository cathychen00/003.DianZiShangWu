...
// 定义消息内容
private string NoFileMessage = "请选择上传文件";
private string UploadSuccessMessage = "上传成功";
private string NoImagesMessage = "无文件";
private string NoFolderSpecifiedMessage = "文件夹不存在";
private string NoFileToDeleteMessage = "未删除文件";
private string InvalidFileTypeMessage = "无效的文件格式";
...
private string	DefaultImageFolder = "image";
            ...
            if (UploadFile.PostedFile.FileName.Trim() != "")
			{
				if (IsValidFileType(UploadFile.PostedFile.FileName))
				{
					try
					{
                        ...
                        //CurrentImagesFolder.Value = CurrentImagesFolder.Value.Replace(@"/", @"\");
                        ...
                        ResultsMessage.Text = UploadFileDestination;
					}
                        
					catch(Exception ex)
					{
						ResultsMessage.Text = "无法上传: " + ex.Message;						
					}
				}
				else
				{
...
public void DisplayImages()
{
	...
	else
	{
		...
		if (CurrentImagesFolder.Value != RootImagesFolder.Value)
		{
			...
			myHtmlImage.Src = AppUrl + "image/folder.up.gif";
		...
		foreach (string _Directory in DirectoriesArray)
		{
			...
			try {
				...
				System.Web.UI.HtmlControls.HtmlImage myHtmlImage = new System.Web.UI.HtmlControls.HtmlImage();
				myHtmlImage.Src = AppUrl + "image/folder.big.gif";
				myHtmlImage.Attributes["unselectable"]="on"; 
				myHtmlImage.Attributes["align"]="absmiddle"; 
	...
	<HTML>
		<HEAD runat="server" >
			...
			<script language="javascript">
				...
				{
					window.navigate("Dialog.aspx?frame=1&rif=" + rootfolder + "&cif=" + newfolder);
				}		
				function returnImage(imagename,width,height)
				{
					...
					arr["width"] = width;  
					arr["height"] = height;			 
					window.parent.returnValue = arr["filename"];
				//window.returnValue = arr["filename"];
					window.parent.close();	
				}		
			</script>
		</HEAD>
						...
						<tr>
							<td height="16" style="padding-left:10px;border-top: 1 solid #999999; background-color:#99ccff;">
								<table>
									<tr>
										<td valign="top">
										<input id="UploadFile" type="file" name="UploadFile" runat="server" style="width:300;" /></td>
...
<asp:panel id="iframePanel" runat="server">
	<html>
		...
		<body>
			<iframe style="width:100%;height:100%;border:0;" border=0 frameborder=0 src="Dialog.aspx?frame=1&<%=Request.QueryString%>">
			</iframe>
		</body>
	</html>
</asp:panel>
